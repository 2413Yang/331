...
namespace fsm
{
    ...
    inline std::string char2str( char t ) {
        char str_buf[2] = {0};
        str_buf[0] = t;
    ...
    inline int id( const std::string &t ) {
        int val = 0;
        size_t i;
        size_t len = t.length();
        for (i = 0; i < len; i++) {
    ...
    struct state {
        ...
        state( const int &name = fsm::id("null")) : name(name)
        {}
        ...
        inline friend ostream &operator<<( ostream &out, const state &t ) {
            if( t.name >= 256 ) {
                out << fsm::char2str(char((t.name >> 24) & 0xff));
                out << fsm::char2str(char((t.name >> 16) & 0xff));
                out << fsm::char2str(char((t.name >>  8) & 0xff));
                out << fsm::char2str(char((t.name >>  0) & 0xff));
            } else {
                out << fsm::char2str(char(t.name));
            }
            out << std::string("(");
            std::string sep;
            for(auto &arg : t.args ) {
    ...
    public:
        ...
        stack( const fsm::state &start = fsm::id("null") ) : deque(1) {
            deque[0] = start;
            call( deque.back(), fsm::id("init") );
        }
        ...
        void pop() {
            if( deque.size() ) {
                call( deque.back(), fsm::id("quit") );
                deque.pop_back();
            }
            if( deque.size() ) {
                call( deque.back(), fsm::id("back") );
            }
        }
    ...
    protected:
        ...
        void replace( fsm::state &current, const fsm::state &next ) {
            call( current, fsm::id("quit") );
            current = next;
            call( current, fsm::id("init") );
        }
