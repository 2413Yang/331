...
extern std::mutex mLogMutex;

#if LOG_LEVEL >= LOG_LEVEL_INFO
#define LOG_RECORD(format, ...)	do {					\
		char buf[1024];									\
		snprintf(buf, sizeof(buf),format, ##__VA_ARGS__);					\
		std::lock_guard<std::mutex> lock(mLogMutex);	\
		ZH::logService::LogRecord("Upgrade", ZH::logService::EnLogLevel::LOGLEVEL_INFO, buf);	\
} while (0)
#else
#define LOG_RECORD(format, ...)  do {}while(0)
#endif

#if LOG_LEVEL >= LOG_LEVEL_DEBUG
#define LOG_RECORD_DEBUG(format, ...)	do {					\
		char buf[1024];									\
		snprintf(buf, sizeof(buf),format, ##__VA_ARGS__);					\
		std::lock_guard<std::mutex> lock(mLogMutex);	\
		ZH::logService::LogRecord("Upgrade", ZH::logService::EnLogLevel::LOGLEVEL_DEBUG, buf);	\
} while (0)
#else
#define LOG_RECORD_DEBUG(format, ...)  do {}while(0)
#endif
